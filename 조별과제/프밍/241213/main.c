...
/*
printf("\nFinal book list after reading from file:\n");
    book_s *cur = book_head;
    while (cur != NULL) {
        printf("Number : %d | Title : %s | Publisher : %s | \nAuthor : %s | ISBN : %lld | Location : %s | isOK : %c\n",
               cur->booknum, cur->bookname, cur->publisher, cur->author, cur->ISBN, cur->location, cur->available);
        if (cur->next != NULL) { // 마지막 노드가 아니라면 출력
        printf("%lld\n\n", cur->next->ISBN - cur->ISBN);
    }
   cur = cur->next;
    }
*/
